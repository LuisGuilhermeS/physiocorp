...
<body class="container mt-4">
  ...
  <div class="d-flex justify-content-between mb-3">
    <a href="?mes=<?= $mes - 1 ?>&ano=<?= $mes == 1 ? $ano - 1 : $ano ?>" class="btn btn-outline-primary">← Mês Anterior</a>
    <h3><?= strftime('%B de %Y', $data) ?></h3>
    <a href="?mes=<?= $mes + 1 ?>&ano=<?= $mes == 12 ? $ano + 1 : $ano ?>" class="btn btn-outline-primary">Próximo Mês →</a>
  </div>
        ...
        for ($linha = 0; $linha < 6; $linha++) {
          ...
          for ($coluna = 0; $coluna < 7; $coluna++) {
            if (($linha == 0 && $coluna < $primeiroDiaSemana) || $dia > $diasNoMes) {
              ...
              echo "<td><button class='btn btn-sm btn-outline-secondary w-100' onclick='carregarHorarios($dia, $mes, $ano)'>$dia</button></td>";
              $dia++;
            }
          }
          echo "</tr>";
